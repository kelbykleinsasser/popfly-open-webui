...
class OAuthManager:
    ...
    async def _fetch_google_groups(self, email: str, access_token: str) -> list:
        """Fetch Google Groups for a user using the Admin SDK API.
        
        Args:
            email: User's email address
            access_token: OAuth access token with admin.directory.group.member.readonly scope
            
        Returns:
            List of group names the user is a member of
        """
        if not access_token:
            log.warning("No access token provided for Google Groups fetching")
            return []
            
        try:
            groups_url = f"https://admin.googleapis.com/admin/directory/v1/groups?userKey={email}"
            headers = {
            ...
            async with aiohttp.ClientSession(trust_env=True) as session:
                async with session.get(
                    ...
                    if resp.ok:
                        ...
                        group_names = [group.get("name", group.get("email", "")) for group in groups]
                        log.debug(f"Fetched {len(group_names)} groups for user {email}: {group_names}")
                        return group_names
                    else:
                        error_text = await resp.text()
                        log.warning(f"Failed to fetch Google Groups for {email}: {resp.status} - {error_text}")
                        return []
        except Exception as e:
            log.error(f"Error fetching Google Groups for {email}: {e}")
            return []
    ...
    async def handle_callback(self, request, provider, response):
        ...
        # For Google provider, fetch groups using Admin SDK API
        if provider == "google" and auth_manager_config.ENABLE_OAUTH_GROUP_MANAGEMENT:
            email = user_data.get(auth_manager_config.OAUTH_EMAIL_CLAIM, "")
            if email:
                access_token = token.get("access_token")
                google_groups = await self._fetch_google_groups(email, access_token)
                if google_groups:
                    # Add groups to user_data for processing by existing group management logic
                    user_data["groups"] = google_groups
                    log.debug(f"Added Google Groups to user_data: {google_groups}")
